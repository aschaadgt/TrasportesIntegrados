...
function buildExecutiveQuoteEmail({ location, rangeStart, rangeEnd, pickupTime, vehicleType }) {
  ...
  const subject = "Solicitud de cotización - Piloto ejecutivo";
  const body = [
    "Hola, deseo cotizar el servicio de piloto ejecutivo.",
    "",
...
if (
  ...
  const monthFormatter = new Intl.DateTimeFormat("es-GT", {
    month: "long",
    year: "numeric",
  });
  ...
  const emailDateFormatter = new Intl.DateTimeFormat("es-GT", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  function isSameDay(firstDate, secondDate) {
    return firstDate && secondDate && dateKey(firstDate) === dateKey(secondDate);
  }

  function updateDateSummary() {
    if (rangeStart && rangeEnd) {
      dateRangeValue.textContent = `${shortDateFormatter.format(rangeStart)} – ${shortDateFormatter.format(rangeEnd)}`;
      startDateInput.value = dateKey(rangeStart);
      endDateInput.value = dateKey(rangeEnd);
      confirmDates.disabled = false;
      calendarHelp.textContent = "Rango seleccionado. Confirma para continuar.";
    ...
    if (rangeStart) {
      ...
      startDateInput.value = dateKey(rangeStart);
      endDateInput.value = "";
      confirmDates.disabled = true;
      calendarHelp.textContent = "Ahora selecciona la fecha final.";
      return;
    }

    dateRangeValue.textContent = "Seleccionar fechas";
    startDateInput.value = "";
    endDateInput.value = "";
    confirmDates.disabled = true;
    calendarHelp.textContent = "Selecciona la fecha de inicio y después la fecha final.";
  }
  ...
  function renderCalendar() {
    ...
    for (let index = 0; index < totalCalendarDays; index += 1) {
      ...
      dayButton.dataset.date = dateKey(day);
      dayButton.setAttribute("role", "gridcell");
      dayButton.setAttribute("aria-label", emailDateFormatter.format(day));
  ...
  document.addEventListener("click", (event) => {
    if (!event.composedPath().includes(dateRangeField)) {
      closeCalendar();
    }
  });

  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !dateRangeCalendar.hidden) {
      closeCalendar();
  ...
  quoteForm.addEventListener("submit", (event) => {
    ...
    if (!rangeStart || !rangeEnd) {
      dateRangeField.classList.add("is-invalid");
      dateRangeError.textContent = "Selecciona la fecha de inicio y final.";
      openCalendar();
      return;
    }
